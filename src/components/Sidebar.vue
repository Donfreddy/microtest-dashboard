<template>
  <div class="flex">
    ...
    <div
        ...
        @click="isOpen = false"
        class="fixed inset-0 z-20 transition-opacity bg-black opacity-25 lg:hidden"
    ></div>
    <!-- End Backdrop -->

    <div
        :class="isOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
        class="fixed inset-y-0 left-0  z-30 w-64 overflow-y-auto transition duration-300 transform bg-white border-r border-gray-200 text-gray-900 lg:translate-x-0 lg:static lg:inset-0"
    >
      <div class="flex items-center justify-center mt-20">
        <div class="w-2/3 text-center font-bold">
          <a href="#" class="text-2xl font-bold flex items-center lg:ml-2.5">
            ...
            <span class="self-center whitespace-nowrap">Microtest</span>
          </a>
        </div>
      </div>
      ...
      <nav class="mt-20">
        ...
        <router-link
            class="flex items-center px-6 py-2 mt-2 duration-200 border-l-4"
            :class="[$route.name === 'AddUser' ? activeClass : inactiveClass]"
            to="/add-user"
        >
          <svg
              xmlns="http://www.w3.org/2000/svg"
...
export default defineComponent({
  setup() {
    ...
    const inactiveClass = ref(
        'border-white text-gray-500 hover:bg-gray-100 hover:text-gray-900 transition-colors',
    );
